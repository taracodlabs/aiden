...
- **Execution sandbox** — file tools enforce an allowlist (cwd, `~/Documents`, `~/Downloads`, `~/Desktop`, OS temp) with a denylist that always wins (`~/.ssh`, `~/.aws`, `/etc`, `/var`, …); `shell_exec` routes through a long-lived hardened Docker container (`--cap-drop ALL`, `--security-opt no-new-privileges`, memory/CPU/pids caps, `--tmpfs` for `/tmp /var/tmp /run`) and falls back to local with a one-time warning when Docker isn't reachable. A new `sandbox_violation` failure category feeds the recovery engine with auto-derived `AIDEN_SANDBOX_ALLOW` override hints. Optional `AIDEN_DRYRUN=1` mode returns a structured preview ("would create file X, would run command Y") instead of executing — opt-in. Active by default; set `AIDEN_SANDBOX=0` to disable. Extend the allowlist via `AIDEN_SANDBOX_ALLOW=path1:path2`.
