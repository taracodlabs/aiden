...
describe.each([100, 44])('busy composer rendered screen at %i columns', (columns) => {
  it('remains pinned after two durable queue acknowledgements', () => {
    ...
    expect(screen.lines().at(-5)).toContain('▲ You · queue mode');
    expect(screen.lines().at(-4)).toBe('─'.repeat(21));
    expect(screen.lines().at(-3)).toBe('');
    expect(screen.lines().at(-2)).toMatch(/^─+$/u);
