...
describe('npm publish config', () => {
  it('1. declares the public runtime package and complete release edition', async () => {
    ...
    expect(pkg.aiden?.edition).toBe('pro');
    expect(pkg.files).toContain('LICENSE-SKILLS.md');
    expect(pkg.scripts?.prepublishOnly).not.toContain('verify-commercial-publish.mjs');
  ...
  it('2. has no auto-publish workflow and keeps manual publication canonical', async () => {
    ...
    // Manual publication remains the canonical, authentication-gated path.
